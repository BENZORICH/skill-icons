...
function generateSvg(iconNames, perLine) {
  ...
  const length = Math.min(perLine * 300, iconSvgList.length * 300) - 44;
...
>
  ${iconSvgList
    .map((i, index) => {
      const x = (index % perLine) * 300;
      const y = Math.floor(index / perLine) * 300;
      return `
  ...
  return names
    .map(name => {
      if (iconNameList.includes(name)) {
        ...
        const mapped = shortNames[name];
        return mapped + (themedIcons.includes(mapped) ? `-${theme}` : '');
      }
      return null;
    })
    .filter(Boolean);
...
async function handleRequest(request) {
  ...
  if (path === '') {
    return Response.redirect('https://github.com/BENZORICH/skill-icons', 302);
  }
...
addEventListener('fetch', event => {
  event.respondWith(
    handleRequest(event.request).catch(
      err => new Response(err.stack || String(err), { status: 500 })
    )
  );
});